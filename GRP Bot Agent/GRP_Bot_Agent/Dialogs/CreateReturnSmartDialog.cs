...
public class CreateReturnSmartDialog : ComponentDialog
{
    private readonly StateService _stateService;

    public CreateReturnSmartDialog(string dialogId, StateService stateService)
        : base(dialogId)
    {
    ...
    { 
        ...
        AddDialog(new WaterfallDialog($"{nameof(CreateReturnSmartDialog)}.mainFlow", waterfallSteps));
        AddDialog(new TextPrompt($"{nameof(CreateReturnSmartDialog)}.returnItems"));
        AddDialog(new ConfirmPrompt(nameof(ConfirmPrompt)));

        InitialDialogId = $"{nameof(CreateReturnSmartDialog)}.mainFlow";
    }
    ...
    private async Task<DialogTurnResult> AskOrderItemsSelectionStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
    {
        ...
        await DisplayItemsAsCarouselAsync(stepContext.Context, orderItemsAvailableForReturn, cluResult, cancellationToken);
    ...
    public async Task DisplayItemsAsCarouselAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var selectionsState = await _stateService.OrderItemSelectionsAccessor.GetAsync(turnContext, () => new Dictionary<string, List<OrderItemSelection>>(), cancellationToken);
        var first = selectionsState.Values.FirstOrDefault();
        first?.Clear();
        ...
        //only allow one item to be matched with the CLU response for now.
        var alreadyMatched = false;

        foreach (var item in orderItems)
        {
            var isMatch = !string.IsNullOrEmpty(cluResponse?.ProductDescription) && Fuzz.Ratio(item.ProductDescription, cluResponse.ProductDescription) > 50;
            ...
            var adaptiveCard = new AdaptiveCard(new AdaptiveSchemaVersion(1, 3))
            {
                Body = new List<AdaptiveElement>
                {
                    ...
                    new AdaptiveToggleInput
                    {
                        ...
                        ValueOn = "true",
                        ValueOff = "false",
                        Value = isMatch && !alreadyMatched ? "true" : "false"
                    },
                    new AdaptiveChoiceSetInput
                    {
                        ...
                        IsMultiSelect = false,
                        Value = isMatch && !alreadyMatched ? cluResponse.ReturnReason : null,
            ...
            if (isMatch && !alreadyMatched)
            {
                ...
                {
                    var selection = userItemSelections.FirstOrDefault(s => s.ItemId == item.Id);
                    if (selection == null)
                    {
                        userItemSelections.Add(new OrderItemSelection { ItemId = item.Id, ReturnReasonCode = cluResponse.ReturnReason });
                    }
                }
                else
                {
                    selectionsState.Add(turnContext.Activity.From.Id,
                        new List<OrderItemSelection>
                        {
                            ...
                            {
                                ItemId = item.Id,
                                ReturnReasonCode = cluResponse.ReturnReason
                            }
                        });
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await client.CreateReturnOrderWithHttpMessagesAsync(logInData.TenantCode,
            ...
            new ReturnsOmsApiUseCasesCreateReturnOrderCreateReturnOrderWebRequest
            {
                ...
                ShippingReference = logInData.Order.Items.First(i => i.Id == selectionsState.First().Value.Last().ItemId).ShippingInformation.ShippingReference,
